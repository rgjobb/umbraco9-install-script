# Custom version of iis-builder https://github.com/mattou07/iis-builder
...
# Ensure the script is elevated to Admin permissions
...
$iisls1 = '"iis": {
	  "applicationUrl": "https://'+$sdname+'.localtest.me",
	  "sslPort": 0
	},
	"iisExpress":'
...
$sourceFile = $path+'\Properties\launchSettings.json'
$destinationFile =  $sourceFile

$hasIISConfig = Select-String -Path $sourceFile -Pattern '"IIS"'

if ($hasIISConfig -eq $null)
{
    (Get-Content $sourceFile) | ForEach-Object {
		$_.replace('"iisExpress":', $iisls1).replace('"Umbraco.Web.UI":', $iisls2)
...
$iis = [pscustomobject]@{
    siteName = $name
    appPoolName = $name
    siteBindings = "$sdname.localtest.me"
    dir = $dir
    dotNetVersion = ""
}	
